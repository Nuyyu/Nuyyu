...
exports.run = async(client, message, args) => {
  ...
  let uyarisi = await db.fetch(`uyarı.${kisi.id + message.guild.id}`)
  if (!uyarisi || uyarisi == 0) return message.channel.send(yok)
        let uyarisayiyolla = new Discord.RichEmbed()
        .setColor("RANDOM")
        .setDescription(`**${kisi.username} Adlı Kullanıcının Toplam Uyarısı : **` + db.fetch(`uyarı.${kisi.id+message.guild.id}`) || '0', true)
